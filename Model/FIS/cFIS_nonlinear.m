function [Y_output, cnsqParm] = cFIS_nonlinear(H, Y, baseVarFuzzyN, ifParm)
    HNum = size(H,1);
    ...
    %initialize RLSE parameters (修改點1：維度變為 2N+1)
    numOfCoeff = size(H,2)*2 + 1;
    ...
    for i=1:HNum
        h = HT(:,i);
        % 修改點2：擴展為 [1; h1; h1^2; h2; h2^2; ...]
        h_ext = [1; reshape([h, h.^2]', [], 1)]; 
        b = reshape(h_ext*nfs(i,:),[ruleNum*numOfCoeff,1]);
